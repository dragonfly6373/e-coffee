function DateTimePicker(n) {
    ...
    this.withTimeInput = n.getAttribute("with-time") == "true";
    ...
    this.bind("click", function () {
        ...
        this.pickerPopup.toggle(this.node(), "left-inside", "bottom", 0, 5, "autoFlip");
    }, this.pickerButton);
...
DateTimePicker.prototype.getFormat = function () {
    var javaFormat = this.format || DateTimePicker.GLOBAL_FORMAT || (this.withTime() ? DateTimePicker.DEFAULT_DATE_TIME_FORMAT : DateTimePicker.DEFAULT_DATE_FORMAT);
...
DateTimePicker.prototype.isInRange = function(m) {
    var inRange = false;
    if (!this._minDate && !this._maxDate) {
        inRange = true;
    } else if (this._minDate && this._maxDate) {
        inRange = m.isBetween(this._minDate, this._maxDate);
    } else if (this._minDate) {
        inRange = m.isAfter(this._minDate);
    } else {
        inRange = m.isBefore(this._maxDate);
    }
    return inRange;
}
...
DateTimePicker.prototype.setMinDate = function(date) {
    if (!date) delete this._minDate;
    this._minDate = moment(date).add(-1, "days");
    this.invalidate();
}
DateTimePicker.prototype.setMaxDate = function(date) {
    if (!date) delete this._maxDate;
    this._maxDate = moment(date).add(1, "days");
    this.invalidate();
}
